...
async function loadManifest() {
  if (!manifest) {
    manifest = JSON.parse(await fs.readFile(path.resolve(manifestPath), "utf-8"));
  }
  return manifest;
}

/**
 * Renders the React application on the server and streams the HTML to the client.
 *
 * @param {object} params - Parameters for rendering.
 * @param {import("vite").ViteDevServer} [params.vite] - Vite dev server instance (dev only).
 * @param {boolean} params.isProduction - True in production mode.
 ...
 * @param {object|null} params.mutationPayload - Payload for displaying toasts after redirect.
 * @param {string} params.invalidateCacheCookie - Cookie to invalidate the entry-server suspense cache.
 * @param {import("http").ServerResponse} res - HTTP response to stream the HTML to.
 * @returns {Promise<void>} A promise that resolves when rendering and streaming complete.
 */
export async function renderApp(
  { vite, isProduction, url, locale, pageName, frontendMode, mutationPayload, invalidateCacheCookie },
  res,
) {
  try {
    let render;
    ...
    if (!isProduction) {
      ...
      for (const [, chunk] of Object.entries(productionManifest)) {
        if (chunk.css) {
    ...
    const writable = new Writable({
      ...
      final(callback) {
        const postludeContent =
          typeof rendered.postlude === "function" ? rendered.postlude() : rendered.postlude;
